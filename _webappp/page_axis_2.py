...
st.write("""
          Frequency tells us what is common, but not what actually works as a joke. To asess the performance of occupations we can check which occupations are actually the funniest, or the worst performers. This can be done by looking at the median funny scores, introduced in axis 1, of captions that reference each occupation above a certain threshold frequency to avoid noise. The results show us that it is indeed not the most frequent occupations that are present in the funniest captions: only "rabbi" made both cuts while the term "leader" was also among the worst performers. In each of the charts, we see a very small gradual change along the x-axis, indicating that there are no occupations that are dramatically funnier or less funny than others. Furthermore, we see that the difference between the best and worst occupations is quite small, suggesting that while some jobs might be slightly funnier than others on average, the overall impact of occupation on humour is limited.
         """)
...
st.write("""
        ...
        This is a useful reminder for reading these plots in a data story. We are not simply asking which category is funniest on average. We are asking how our humour behaves. The clusters near the lower scores suggest that job titles alone rarely carry a joke. The higher outliers suggest that when an occupation based caption works, it tends to work because it adds something else, such as a recognisable stereotype, a surprising role reversal, or a sharp twist on workplace culture.

        There is a practical implication too. These categories do not appear equally often in the dataset. Only five categories have more than 10,000 captions that mention them, so some boxes are built from much smaller evidence. This can explain the apparently less outliers in categories like Sports and Fitness or Domestic and Personal Care. With fewer data points, there are simply fewer chances for a caption to break out as a high scoring joke.
""")
